...
import { getPokemonColor, Pokemon, POKEMON_RULES } from '../../pokemon.models';
...
export class PokemonAddComponent {
  ...
  onSubmit() {
    ...
    const pokemon: Omit<Pokemon, 'id'> = {
      ...
      domage: this.pokemonDamage.value,
      types: (() => {
        const types = this.pokemonTypeList.controls.map(control => control.value);
        if (types.length === 1) return [types[0]] as [string];
        if (types.length === 2) return [types[0], types[1]] as [string, string];
        if (types.length === 3) return [types[0], types[1], types[2]] as [string, string, string];
